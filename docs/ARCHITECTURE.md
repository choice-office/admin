...
## 홈 노출 (`/home`) — 쇼츠 4칸 + 대표글 4칸 + 후기 8~12개
...
### ③ 의뢰인이 직접 전한 후기 (마퀴)
- 데이터: `review_images.is_featured` — **후기 관리와 같은 표, 스키마 변경 없음**. 순서는 후기 관리의 `sort_order` 를 그대로 따른다(여기서 순서를 다시 정하지 않는다).
- **칸이 고정되어 있지 않다.** 마퀴에 흘리는 가변 목록이라 칸 모달이 아니라 **추가/삭제**로 다룬다:
  - `[후기 추가]` → `ReviewPickerModal`(노출 중이고 아직 안 고른 후기 · 검색 · 페이지네이션). 모달을 닫지 않고 여러 개 연달아 추가할 수 있고 남은 자리를 실시간으로 보여준다.
  - 카드마다 순번(1~N)과 `✕` 빼기 버튼.
- **최소 8개 · 최대 12개**: 마퀴가 짧으면 반복이 눈에 보이고, 길면 홈이 무거워진다. 8개에서 빼기 차단 · 12개에서 추가 차단(버튼 비활성 + 모달 행 클릭 차단).
  - 노출 중인 후기 자체가 8개 미만이면 상·하한을 지킬 수 없다 → 화면에 사실대로 알린다.
  - 공개 렌더(홈페이지 `lib/review-images.ts`)도 같은 규칙으로 **8개보다 적으면 남은 노출 후기로 채운다** → 홈은 항상 8~12개.
- ★ 모달의 남은 자리는 **부모의 현재 개수만으로** 센다. 자체 카운터를 더하면 부모가 추가 후 refetch 하면서 이중 계산돼 자리가 두 칸씩 줄어든다(실제로 겪은 버그).
- 후기 관리(`/reviews`)에는 **`★홈` 읽기 전용 배지**만 있다(별표 토글 제거) — 고르는 것은 여기서만.
